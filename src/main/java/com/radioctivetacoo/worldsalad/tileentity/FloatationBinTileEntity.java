...
public class FloatationBinTileEntity extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
	...
	public FloatationBinTileEntity(TileEntityType<?> tileEntityTypeIn) {
		super(tileEntityTypeIn);

		this.inventory = new WorldSaladItemHandler(5);
	}

	public FloatationBinTileEntity() {
		this(TileEntityInit.FLOATATION_BIN.get());
	}
	...
	public Container createMenu(final int windowID, final PlayerInventory playerInv, final PlayerEntity playerIn) {
		return new FloatationBinContainer(windowID, playerInv, this);
	}
	...
	public void tick() {
		...
		if (this.world != null && !this.world.isRemote) {
			...
			if (this.inventory.getStackInSlot(2).getItem().equals(ItemInit.PINE_OIL_BUCKET.get())
			...
			if (this.getRecipe(this.inventory.getStackInSlot(0)) != null && this.isPowered() && this.fluidLeft >= 20 && this.inventory.getStackInSlot(1).getCount() < 64 && this.inventory.getStackInSlot(4).getCount() < 64) {
				ItemStack output = this.getRecipe(this.inventory.getStackInSlot(0)).getRecipeOutput();
				if ((this.inventory.getStackInSlot(1).getItem().equals(output.getItem()) || this.inventory.getStackInSlot(1).isEmpty()) && (this.inventory.getStackInSlot(4).getItem().equals(output.getItem()) || this.inventory.getStackInSlot(4).isEmpty())) {
			...
			if (this.currentSmeltTime == this.maxSmeltTime) {
				this.currentSmeltTime = 0;
				this.fluidLeft = this.fluidLeft - 20;
				ItemStack output = this.getRecipe(this.inventory.getStackInSlot(0)).getRecipeOutput();
				this.inventory.insertItem(1, output.copy(), false);
				this.inventory.insertItem(4, output.copy(), false);
				this.inventory.decrStackSize(0, 1);
				this.world.playSound((PlayerEntity)null, this.pos, SoundEvents.ENTITY_BOAT_PADDLE_WATER, SoundCategory.BLOCKS, 0.7F, 0.8F);
				dirty = true;
			}
	...
	private ITextComponent getDefaultName() {
		return new TranslationTextComponent("container." + WorldSalad.MOD_ID + ".floatation_bin");
	}
	...
	@Nullable
	private FloatationBinRecipe getRecipe(ItemStack stack) {
		if (stack == null) {
			return null;
		}

		Set<IRecipe<?>> recipes = findRecipesByType(RecipeSerializerInit.FLOATATION_BIN_TYPE, this.world);
		for (IRecipe<?> iRecipe : recipes) {
			FloatationBinRecipe recipe = (FloatationBinRecipe) iRecipe;
