...
public class HydraulicPressTileEntity extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
	...
	public final int maxSmeltTime = 300;
	private WorldSaladItemHandler inventory;

	public HydraulicPressTileEntity(TileEntityType<?> tileEntityTypeIn) {
	...
	public HydraulicPressTileEntity() {
		this(TileEntityInit.HYDRAULIC_PRESS.get());
	}
	...
	public Container createMenu(final int windowID, final PlayerInventory playerInv, final PlayerEntity playerIn) {
		return new HydraulicPressContainer(windowID, playerInv, this);
	}
	...
	public void tick() {
		...
		if (this.world != null && !this.world.isRemote) {
			if (this.inventory.getStackInSlot(0).isEmpty() || this.inventory.getStackInSlot(1).isEmpty() || this.energyLeft == 0) {
				this.currentSmeltTime = 0;
			}
			if (this.inventory.getStackInSlot(4).getItem().equals(ItemInit.FLUX_POWDER.get()) && this.energyLeft + 200 <= this.energyMax) {
				this.inventory.decrStackSize(4, 1);
				this.energyLeft = this.energyLeft + 200;
			}
			if (this.inventory.getStackInSlot(3).getItem().equals(Items.WATER_BUCKET)
			...
			if (this.getRecipe(this.inventory.getStackInSlot(0)) != null && this.getRecipe(this.inventory.getStackInSlot(1)) != null && this.isPowered() && this.fluidLeft >= 20 && this.inventory.getStackInSlot(2).getCount() < 64) {
			...
			if (this.currentSmeltTime == this.maxSmeltTime) {
				...
				this.world.playSound((PlayerEntity)null, this.pos, SoundEvents.BLOCK_ANVIL_PLACE, SoundCategory.BLOCKS, 0.7F, 0.8F);
				dirty = true;
			}
	...
	private ITextComponent getDefaultName() {
		return new TranslationTextComponent("container." + WorldSalad.MOD_ID + ".hydraulic_press");
	}
	...
	@Nullable
	private HydraulicPressRecipe getRecipe(ItemStack stack) {
		if (stack == null) {
			return null;
		}

		Set<IRecipe<?>> recipes = findRecipesByType(RecipeSerializerInit.HYDRAULIC_PRESS_TYPE, this.world);
		for (IRecipe<?> iRecipe : recipes) {
			HydraulicPressRecipe recipe = (HydraulicPressRecipe) iRecipe;
