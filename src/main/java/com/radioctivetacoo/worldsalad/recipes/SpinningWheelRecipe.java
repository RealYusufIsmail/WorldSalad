...
public class SpinningWheelRecipe implements ISpinningWheelRecipe {
	...
	public SpinningWheelRecipe(ResourceLocation id, Ingredient input, Ingredient input2, ItemStack output) {
		this.id = id;
		this.output = output;
		this.input = input;
		this.input2 = input2;
	...
	public boolean matches(RecipeWrapper inv, World worldIn) {
		if (this.input.test(inv.getStackInSlot(0)) && this.input2.test(inv.getStackInSlot(1)) && this.input2.test(inv.getStackInSlot(2)) && this.input2.test(inv.getStackInSlot(3))) {
			return true;
		}
		return false;
	}
