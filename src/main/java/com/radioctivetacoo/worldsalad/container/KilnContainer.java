...
import com.radioctivetacoo.worldsalad.container.slots.ModFuelSlot;
...
public class KilnContainer extends Container {
	...
	public KilnTileEntity tileEntity;
	...
	// Server Constructor
	public KilnContainer(final int windowID, final PlayerInventory playerInv, final KilnTileEntity tile) {
		super(ContainerInit.KILN.get(), windowID);
		...
		this.addSlot(new ModFuelSlot(tile.getInventory(), 1, 56, 53));
		this.addSlot(new ModOutputSlot(tile.getInventory(), 2, 116, 35));
	...
	// Client Constructor
	public KilnContainer(final int windowID, final PlayerInventory playerInv, final PacketBuffer data) {
		this(windowID, playerInv, getTileEntity(playerInv, data));
	}

	private static KilnTileEntity getTileEntity(final PlayerInventory playerInv, final PacketBuffer data) {
		Objects.requireNonNull(playerInv, "playerInv cannot be null");
		Objects.requireNonNull(data, "data cannot be null");
		final TileEntity tileAtPos = playerInv.player.world.getTileEntity(data.readBlockPos());
		if (tileAtPos instanceof KilnTileEntity) {
			return (KilnTileEntity) tileAtPos;
		}
	...
	public boolean canInteractWith(PlayerEntity playerIn) {
		return isWithinUsableDistance(canInteractWithCallable, playerIn, BlockInit.KILN.get());
	}
