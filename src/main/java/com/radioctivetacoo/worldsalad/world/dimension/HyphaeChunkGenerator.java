...
public class HyphaeChunkGenerator extends NoiseChunkGenerator<HyphaeGenSettings> {
	...
	public HyphaeChunkGenerator(IWorld worldIn, BiomeProvider provider, HyphaeGenSettings settingsIn) {
	...
	protected void fillNoiseColumn(double[] noiseColumn, int noiseX, int noiseZ) {
		...
		int i = -10;
		int j = 3;
		this.calcNoiseColumn(noiseColumn, noiseX, noiseZ, 684.412F, 684.412F, 8.555149841308594D,
	...
	protected double func_222545_a(double p_222545_1_, double p_222545_3_, int p_222545_5_) {
		double d0 = 8.5D;
		double d1 = (p_222545_5_ - (8.5D + p_222545_1_ * 8.5D / 8.0D * 4.0D)) * 12.0D * 128.0D / 256.0D
				/ p_222545_3_;
		if (d1 < 0.0D) {
			d1 *= 4.0D;
		}
	...
	private double getNoiseDepthAt(int noiseX, int noiseZ) {
		double d0 = this.depthNoise.getValue(noiseX * 200, 10.0D, noiseZ * 200, 1.0D, 0.0D, true)
				* 65535.0D / 8000.0D;
		if (d0 < 0.0D) {
			d0 = -d0 * 0.3D;
		}
