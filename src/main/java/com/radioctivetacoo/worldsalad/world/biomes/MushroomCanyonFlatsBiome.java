...
public class MushroomCanyonFlatsBiome extends Biome {
	...
	BlockClusterFeatureConfig PRICKLEWEED = (new BlockClusterFeatureConfig.Builder(new SimpleBlockStateProvider(BlockInit.PRICKLEWEED.get().getDefaultState()), new SimpleBlockPlacer())).tries(21).func_227317_b_().build();
	...
	public MushroomCanyonFlatsBiome(Builder biomeBuilder) {
		...
		this.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION,
				Feature.RANDOM_PATCH.withConfiguration(PRICKLEWEED).withPlacement(
						Placement.COUNT_CHANCE_HEIGHTMAP.configure(new HeightWithChanceConfig(10, 0.05F))));
